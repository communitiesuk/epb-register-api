...
deploy-app: ## Deploys the app to PaaS
	...
	cf set-env "${DEPLOY_APPNAME}" VALID_DOMESTIC_SCHEMAS "${VALID_DOMESTIC_SCHEMAS}"
	cf set-env "${DEPLOY_APPNAME}" VALID_NON_DOMESTIC_SCHEMAS "${VALID_NON_DOMESTIC_SCHEMAS}"
	cf set-env "${DEPLOY_APPNAME}" SLACK_EPB_BOT_TOKEN "${SLACK_EPB_BOT_TOKEN}"
...
deploy-worker:
	...
	cf set-env "${DEPLOY_WORKER}" SENTRY_DSN "${WORKER_SENTRY_DSN}"
