    ...
    def execute(assessment_id, assessment_data)
      ...
      assessment =
        Domain::DomesticEnergyAssessment.new(
            ...
            current_space_heating_demand: assessment_data[:heat_demand][:current_space_heating_demand],
            current_water_heating_demand: assessment_data[:heat_demand][:current_water_heating_demand],
            impact_of_loft_insulation: assessment_data[:heat_demand][:impact_of_loft_insulation],
            impact_of_cavity_insulation: assessment_data[:heat_demand][:impact_of_cavity_insulation],
            impact_of_solid_wall_insulation: assessment_data[:heat_demand][:impact_of_solid_wall_insulation],
            recommended_improvements: assessment_data[:recommended_improvements].map do |i|
                                        Domain::RecommendedImprovement.new(
                                          assessment_id: assessment_id,
                                          sequence: i[:sequence],
                                          improvement_code: i[:improvement_code],
                                          indicative_cost: i[:indicative_cost],
                                          typical_saving: i[:typical_saving] ? i[:typical_saving].round(2) : 0,
                                          improvement_category: i[:improvement_category],
                                          improvement_type: i[:improvement_type],
                                          energy_performance_rating_improvement: i[:energy_performance_rating_improvement],
                                          environmental_impact_rating_improvement: i[:environmental_impact_rating_improvement],
