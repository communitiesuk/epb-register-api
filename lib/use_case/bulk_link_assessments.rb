module UseCase
  class BulkLinkAssessments
    def initialize(fetch_assessments_to_link_gateway:, address_base_gateway:, assessment_address_id_gateway:)
      @fetch_assessments_to_link_gateway = fetch_assessments_to_link_gateway
      @address_base_gateway = address_base_gateway
      @assessment_address_id_gateway = assessment_address_id_gateway
    end
    ...
    def execute
      ...
      (1..max_group_id).each do |group_id|
        ...
        @assessment_address_id_gateway.update_assessments_address_id_mapping(domain_object.get_assessment_ids, domain_object.best_address_id, "epb_bulk_linking")
      end
    end
  end
end
