...
const {
  newerVersions, latestVersion, specificVersion, downloadFileUrlForVersionUrl,
  parseVersionNumber
...
program
  ...
  .option('-s, --use-specific-version', 'Load a specific version')
  .action(updateAction)
...
async function updateAction ({ interactive, verbose, forceFull, useSpecificVersion }) {
  ...
  try {
    ...
    console.log(useSpecificVersion)

    if (useSpecificVersion && !forceFull) {
      versionsToUpdate = await specificVersion(versionNumber)
    } else if (currentVersion && !forceFull) {
      versionsToUpdate = await newerVersions(currentVersion)
    } else {
      versionsToUpdate = [await latestVersion()]
    }
      ...
      readStream
        ...
        .on('end', async () => {
          ...
          if (remainingFiles.length > 0) {
            ...
            // I think this newVersion.productVersion is the wrong version to write in
            await swapInNewVersion(versionsToUpdate[versionIndex].productVersion)
