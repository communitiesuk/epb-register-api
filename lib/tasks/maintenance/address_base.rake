...
task :import_address_base do
  ...
  Dir.chdir File.expand_path("~/Downloads/ab_july21/unzipped/")
  ...
  iterations.times do |iteration|
    ...
    csv_contents = CSV.new(file, headers: ADDRESS_BASE_HEADERS)
    puts "[#{Time.now}] #{file_name} parsed successfully as CSV"
    ...
    csv_contents.each_slice(INSERT_BATCH_SIZE).reject(&:empty?).each do |inserts|
      ...
      i += INSERT_BATCH_SIZE
      puts "[#{Time.now}] Inserted #{i} addresses from #{file_name}"
    end
