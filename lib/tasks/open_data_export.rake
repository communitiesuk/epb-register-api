...
task :open_data_export, %i[type_of_export] do |_, args|
  ...
  transmit_file(csv_data, type_of_export)
...
def transmit_file(data, type_of_export)
  filename =
    if type_of_export == "for_odc"
      "open_data_export_#{ENV['ASSESSMENT_TYPE'].downcase}_#{DateTime.now.strftime('%F')}_#{get_max_task_id}.csv"
    else
      "test/open_data_export_#{ENV['ASSESSMENT_TYPE'].downcase}_#{DateTime.now.strftime('%F')}_#{get_max_task_id}.csv"
    end
