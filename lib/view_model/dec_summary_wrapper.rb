    ...
    def to_xml
      xml = <<~XML
                      ...
                      >
                        <Report>
                          <Report-Header>
                            ...
                            <Property-Details>
                              <UPRN>#{if @view_model.address_id&.include?('LPRN-')
                                        ''
                                      else
                            ...
                            <Calculation-Details>
                              <Output-Engine>#{@view_model
          .output_engine}</Output-Engine>
                            </Calculation-Details>
                          </Report-Header>
                          <OR-Operational-Rating>
                            <OR-Assessment-Start-Date>#{@view_model
          .or_assessment_start_date}</OR-Assessment-Start-Date>
                            <OR-Assessment-End-Date>#{@view_model
          .or_assessment_end_date}</OR-Assessment-End-Date>
                            <OR-Benchmark-Data>
                              <Benchmarks>#{get_benchmark_xml(
                                @view_model.benchmarks,
                              )}</Benchmarks>
                            </OR-Benchmark-Data>
                            <OR-Energy-Consumption>#{get_or_energy_consumption_xml(
                              @view_model.or_energy_consumption,
                            )}</OR-Energy-Consumption>
                          </OR-Operational-Rating>
                          <Display-Certificate>
                            <DEC-Annual-Energy-Summary>
                              <Annual-Energy-Use-Electrical>#{@view_model
          .annual_energy_summary[
          :electrical
        ]}</Annual-Energy-Use-Electrical>
                              <Annual-Energy-Use-Fuel-Thermal>#{@view_model
          .annual_energy_summary[
          :fuel_thermal
        ]}</Annual-Energy-Use-Fuel-Thermal>
                              <Renewables-Fuel-Thermal>#{@view_model
          .annual_energy_summary[
          :renewables_fuel_thermal
        ]}</Renewables-Fuel-Thermal>
                              <Renewables-Electrical>#{@view_model
          .annual_energy_summary[
          :renewables_electrical
        ]}</Renewables-Electrical>
                              <Typical-Thermal-Use>#{@view_model
          .annual_energy_summary[
          :typical_thermal_use
        ]}</Typical-Thermal-Use>
                              <Typical-Electrical-Use>#{@view_model
          .annual_energy_summary[
          :typical_electrical_use
                            ...
                            <This-Assessment>
                              <Nominated-Date>#{@view_model
          .current_assessment_date}</Nominated-Date>
                              <Energy-Rating>#{@view_model
          .energy_efficiency_rating}</Energy-Rating>
                              <Electricity-CO2>#{@view_model
          .current_electricity_co2}</Electricity-CO2>
                              <Heating-CO2>#{@view_model
          .current_heating_co2}</Heating-CO2>
                              <Renewables-CO2>#{@view_model
          .current_renewables_co2}</Renewables-CO2>
                            </This-Assessment>
                            <Technical-Information>
                              <Main-Heating-Fuel>#{@view_model
