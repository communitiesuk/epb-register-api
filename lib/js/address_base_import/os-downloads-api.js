...
async function newerVersions (versionString) {
  try {
    const { gbPackage, islandsPackage } = await availablePackages()
...
async function availablePackages () {
  ...
  return {
    gbPackage: extractPackageWithProductName(data, 'AddressBase Plus'),
    islandsPackage: extractPackageWithProductName(data, 'AddressBase Plus - Islands')
  }
}

function extractPackageWithProductName (packagesData, productName) {
  return packagesData.filter(pkg => pkg.productName === productName)[0]
}
