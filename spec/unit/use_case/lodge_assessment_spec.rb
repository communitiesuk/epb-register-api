describe UseCase::LodgeAssessment do
  ...
  before do
    ...
    allow(assessor).to receive_messages(scotland_sap_existing_building_qualification: "INACTIVE", scheme_assessor_id: "SPEC000000")
    allow(assessor).to receive_messages(scotland_sap_new_building_qualification: "ACTIVE", scheme_assessor_id: "SPEC000000")
  ...
  describe ".execute" do
    ...
    context "when the Scottish assessment is passed as being migrated" do
      it "calls AssessmentsGateway to save the assessment data to the assessments table" do
        use_case.execute(scottish_data, true, "SAP-Schema-S-19.0.0")
    ...
    context "when the Scottish assessment is passed as being new and not migrated" do
      before do
        allow(assessments_search_gateway).to receive(:search_by_assessment_id).and_return([])
      end

      it "calls AssessmentsGateway to save the assessment data to the assessments table" do
        use_case.execute(scottish_data, false, "SAP-Schema-S-19.0.0")
      ...
      context "when there is a race condition and the insert fails as the assessment has already been inserted by another request" do
        ...
        it "raises a duplicate assessment ID error" do
          expect { use_case.execute(scottish_data, false, "SAP-Schema-S-19.0.0") }.to raise_error(described_class::DuplicateAssessmentIdException)
        end
      end
      ...
      context "when there is a country_id" do
        ...
        it "sends the country to the assessments_country_id gateway" do
          expect(assessments_country_id_gateway).to have_received(:insert).with(assessment_id: data[:assessment_id], country_id: 3, upsert: false, is_scottish: true)
        end
      end

      context "when there is no country_id" do
        before do
          scottish_data[:country_id] = nil
          use_case.execute(scottish_data, false, "SAP-Schema-S-19.0.0")
        end
