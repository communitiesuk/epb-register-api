describe UseCase::GetCountryForPostcodeBackfill do
  ...
  context "when the postcode is not found but is in address base" do
    ...
    it "returns an England country code" do
      lookup = use_case.execute(postcode:)
      expect(lookup.match?).to be true
      expect(lookup.in_wales?).to be false
      expect(lookup.in_england?).to be true
  ...
  context "when the postcode is not found" do
    ...
    it "returns an empty country code" do
      lookup = use_case.execute(postcode:)
      expect(lookup.country_codes.any?).to be false
      expect(address_base_country_gateway).to have_received :lookup_from_postcode
    end
  end
end
