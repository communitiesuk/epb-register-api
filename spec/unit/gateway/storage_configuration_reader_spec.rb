...
describe "Gateway::StorageConfigurationReader" do
  ...
  before { @storage_configuration = Gateway::StorageConfigurationReader.new }
  ...
  context "when VCAP_SERVICES is present and we read PaaS credentials" do
    ...
    let(:credentials) do
      @storage_configuration.get_paas_configuration(INSTANCE_NAME)
    end
  ...
  context "when VCAP_SERVICES is not present and we read PaaS credentials" do
    before { allow(ENV).to receive(:[]).with("VCAP_SERVICES").and_return(nil) }

    it "we get back an exception" do
      expect {
        @storage_configuration.get_paas_configuration(INSTANCE_NAME)
      }.to raise_error("No VCAP_SERVICES environment variable present")
    end
  end
  ...
  context "when AWS_ACCESS_KEY_ID AND AWS_SECRET_ACCESS_KEY are present and we read local credentials" do
    ...
    let(:credentials) do
      @storage_configuration.get_local_configuration(EXPECTED_BUCKET_NAME)
    end
