describe "Acceptance::ScotlandAssessmentStatus", :set_with_timecop do
  ...
  let(:scheme_id) do
    ...
    add_assessor(
      ...
      body: fetch_assessor_stub.fetch_request_body(scotland_rdsap: "ACTIVE"),
    )
  ...
  context "when assessment does not exist" do
    it "responds with 404 not found when cancelling an assessment" do
      response = update_scottish_assessment_status(assessment_id: "0000-0000-0000-0000-0000",
  ...
  context "when assessment exists" do
    ...
    it "responds that the assessment has been cancelled" do
      ...
      expect(JSON.parse(response.body, symbolize_names: true)[:data][:status]).to eq("CANCELLED")
    end
    ...
    it "rejects a request with invalid request body" do
      response = assertive_post(
        "/api/scotland/assessments/0000-0000-0000-0000-0000/status",
