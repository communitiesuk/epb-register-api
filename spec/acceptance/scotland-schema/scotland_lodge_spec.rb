describe "Acceptance::Assessment::Lodge", :set_with_timecop do
  include RSpecRegisterApiServiceMixin

  before do
    Events::Broadcaster.enable!
    add_countries
    add_assessor scheme_id:, assessor_id: "SPEC000000", body: valid_assessor_request_body
    Timecop.freeze(Time.utc(2023, 6, 27))
  end

  after do
    Events::Broadcaster.disable!
    Timecop.return
  end

  let(:scheme_id) { add_scheme_and_get_id }
  let(:valid_rdsap_xml) { Samples.xml "RdSAP-Schema-S-19.0" }
  let(:valid_rdsap_2200_xml) { Samples.xml "RdSAP-Schema-S-22.0.0" }
  let(:valid_rdsap_210_xml) { Samples.xml "RdSAP-Schema-S-21.0" }
  let(:valid_rdsap_180_xml) { Samples.xml "RdSAP-Schema-S-18.0" }
  let(:valid_rdsap_1700_xml) { Samples.xml "RdSAP-Schema-S-17.00" }
  let(:valid_rdsap_161_xml) { Samples.xml "RdSAP-Schema-S-16.1" }
  let(:valid_rdsap_150_xml) { Samples.xml "RdSAP-Schema-S-15.0" }
  let(:valid_rdsap_140_xml) { Samples.xml "RdSAP-Schema-S-14.0" }
  let(:valid_rdsap_120_xml) { Samples.xml "RdSAP-Schema-S-12.0" }
  let(:valid_rdsap_not_scottish_xml) { Samples.xml "RdSAP-Schema-19.0" }
  let(:valid_sap_xml) { Samples.xml "SAP-Schema-S-19.0.0" }
  let(:valid_sap_2000_xml) { Samples.xml "SAP-Schema-S-20.0.0" }
  let(:valid_sap_170_xml) { Samples.xml "SAP-Schema-S-17.0" }
  let(:valid_sap_161_xml) { Samples.xml "SAP-Schema-S-16.1" }
  let(:valid_cepc_800_xml) { Samples.xml "CEPC-S-8.0.0", "cepc" }
  let(:valid_cepc_71_xml) { Samples.xml "CEPC-S-7.1", "cepc" }
  let(:valid_cepc_70_xml) { Samples.xml "CEPC-S-7.0", "cepc" }
  let(:valid_action_plan_800_xml) { Samples.xml "CS63-S-8.0.0", "cs63" }
  let(:valid_action_plan_xml) { Samples.xml "CS63-S-7.0", "cs63" }
  let(:valid_dec_800_xml) { Samples.xml "DECAR-S-8.0.0", "dec" }
  let(:valid_dec_ar_800_xml) { Samples.xml "DECAR-S-8.0.0", "dec-ar" }
  let(:valid_dec_and_ar_800_xml) { Samples.xml "DECAR-S-8.0.0", "dec+ar" }
  let(:valid_dec_70_xml) { Samples.xml "DECAR-S-7.0", "dec" }
  let(:valid_dec_ar_70_xml) { Samples.xml "DECAR-S-7.0", "dec-ar" }
  let(:valid_dec_and_ar_70_xml) { Samples.xml "DECAR-S-7.0", "dec+ar" }
  let(:valid_assessor_request_body) do
    AssessorStub.new.fetch_request_body(
      scotland_dec_and_ar: "ACTIVE",
      scotland_nondomestic_existing_building: "ACTIVE",
      scotland_nondomestic_new_building: "ACTIVE",
      scotland_rdsap: "ACTIVE",
      scotland_sap_existing_building: "ACTIVE",
      scotland_sap_new_building: "ACTIVE",
      scotland_section63: "ACTIVE",
    )
  end

  context "when using the /api/assessments api endpoint" do
    it "rejects the Scottish assessment migration" do
      expect(lodge_assessment(
        assessment_body: valid_rdsap_xml,
        accepted_responses: [403],
        scopes: %w[assessment:migrate],
        auth_data: {
          scheme_ids: [scheme_id],
        },
        schema_name: "RdSAP-Schema-S-19.0",
        migrated: true,
      ).status).to eq(403)
    end

    it "rejects the Scottish assessment lodgement" do
      expect(lodge_assessment(
        assessment_body: valid_rdsap_xml,
        accepted_responses: [403],
        scopes: %w[assessment:lodge],
        auth_data: {
          scheme_ids: [scheme_id],
        },
        schema_name: "RdSAP-Schema-S-19.0",
        migrated: false,
      ).status).to eq(403)
    end
  end

  context "when using the /api/scotland/assessments api endpoint" do
    context "when the client has a migrate Scotland role" do
      context "when migrating a Scottish domestic assessment" do
        let(:migrated_scotland_rdsap_qualification_data) do
          ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first
        end

        context "when the xml is valid with the correct Scottish schema" do
          it "successfully migrates the assessment" do
            expect(lodge_scottish_assessment(
              assessment_body: valid_rdsap_xml,
              accepted_responses: [201],
              scopes: %w[migrate:scotland],
              auth_data: {
                scheme_ids: [scheme_id],
              },
              schema_name: "RdSAP-Schema-S-19.0",
              migrated: true,
            ).status).to eq(201)
          end

          it "has all expected data points present" do
            migrated_expected_rdsap_data = {
              "assessment_id" => "0000-0000-0000-0000-0000",
              "date_of_assessment" => "2023-06-27",
              "date_registered" => "2023-06-27",
              "type_of_assessment" => "RdSAP",
              "current_energy_efficiency_rating" => 79,
              "postcode" => "FK1 1XE",
              "date_of_expiry" => "2033-06-26",
              "address_line1" => "1 Some Street",
              "address_line2" => "",
              "address_line3" => "",
              "address_line4" => "",
              "town" => "Newkirk",
              "scheme_assessor_id" => "SPEC000000",
              "opt_out" => false,
              "address_id" => "LPRN-0000000000",
              "migrated" => true,
              "cancelled_at" => nil,
              "not_for_issue_at" => nil,
              "created_at" => "2023-06-27",
              "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
            }

            response = lodge_scottish_assessment assessment_body: valid_rdsap_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-19.0",
                                                 migrated: "true"

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(migrated_scotland_rdsap_qualification_data).to eq migrated_expected_rdsap_data
          end

          context "when migrating the same assessment ID" do
            before do
              lodge_scottish_assessment assessment_body: valid_rdsap_xml,
                                        accepted_responses: [201],
                                        scopes: %w[migrate:scotland],
                                        auth_data: {
                                          scheme_ids: [scheme_id],
                                        },
                                        schema_name: "RdSAP-Schema-S-19.0",
                                        migrated: "true"
            end

            it "doesn't raise and error" do
              lodge_scottish_assessment assessment_body: valid_rdsap_xml,
                                        accepted_responses: [201],
                                        scopes: %w[migrate:scotland],
                                        auth_data: {
                                          scheme_ids: [scheme_id],
                                        },
                                        schema_name: "RdSAP-Schema-S-19.0",
                                        migrated: "true"
              expect { migrated_scotland_rdsap_qualification_data["migrated"] }.not_to raise_error
            end
          end

          context "when migrating an assessment submitted by an assessor who is now unqualified" do
            let(:rdsap_xml) do
              add_assessor scheme_id:,
                           assessor_id: "UNQU000000",
                           body: AssessorStub.new.fetch_request_body(
                             domestic_rd_sap: "INACTIVE",
                           )

              xml = Nokogiri.XML valid_rdsap_xml

              xml.css("Membership-Number").children.first.content = "UNQU000000"

              xml.to_s
            end

            it "successfully migrates the assessment" do
              response = lodge_scottish_assessment(assessment_body: rdsap_xml,
                                                   accepted_responses: [201],
                                                   scopes: %w[migrate:scotland],
                                                   auth_data: {
                                                     scheme_ids: [scheme_id],
                                                   },
                                                   schema_name: "RdSAP-Schema-S-19.0",
                                                   migrated: true).status

              expect(response).to eq 201
              expect(migrated_scotland_rdsap_qualification_data["migrated"]).to be_truthy
              expect(migrated_scotland_rdsap_qualification_data["scheme_assessor_id"]).to eq "UNQU000000"
            end
          end
        end

        context "when migrating a valid Scottish SAP assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2023-06-27",
            "date_registered" => "2023-06-27",
            "type_of_assessment" => "SAP",
            "current_energy_efficiency_rating" => 91,
            "postcode" => "EH1 2NG",
            "date_of_expiry" => "2033-06-26",
            "address_line1" => "1 LOVELY ROAD",
            "address_line2" => "NICE ESTATE",
            "address_line3" => "",
            "address_line4" => nil,
            "town" => "TOWN",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "0000000001",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_sap_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "SAP-Schema-S-19.0.0",
                                                 migrated: "true"

            sap_data =  ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(sap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish SAP-Schema-S-17.0 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2018-03-02",
            "date_registered" => "2018-03-02",
            "type_of_assessment" => "SAP",
            "current_energy_efficiency_rating" => 84,
            "postcode" => "EH1 2NG",
            "date_of_expiry" => "2028-03-01",
            "address_line1" => "1 LOVELY ROAD",
            "address_line2" => "NICE ESTATE",
            "address_line3" => "Scotlanshire",
            "address_line4" => nil,
            "town" => "TOWN",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000001",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_sap_170_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "SAP-Schema-S-17.0",
                                                 migrated: "true"

            sap_data =  ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(sap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish SAP-Schema-S-16.1 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2014-01-17",
            "date_registered" => "2016-02-02",
            "type_of_assessment" => "SAP",
            "current_energy_efficiency_rating" => 84,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2026-02-01",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => nil,
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_sap_161_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "SAP-Schema-S-16.1",
                                                 migrated: "true"

            sap_data =  ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(sap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-21.0 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2023-06-27",
            "date_registered" => "2023-06-27",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 80,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2033-06-26",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => "",
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_210_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-21.0",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-18.0 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2019-01-30",
            "date_registered" => "2019-01-30",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 51,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2029-01-29",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => "",
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_180_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-18.0",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-17.00 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2016-09-15",
            "date_registered" => "2016-09-27",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 69,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2026-09-26",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => "",
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_1700_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-17.00",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-16.1 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2013-08-14",
            "date_registered" => "2013-08-15",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 49,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2023-08-14",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => nil,
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_161_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-16.1",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-15.0 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2012-07-12",
            "date_registered" => "2012-07-18",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 74,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2022-07-17",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => "",
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_150_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-15.0",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-14.0 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2010-09-21",
            "date_registered" => "2010-09-23",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 59,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2020-09-22",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => "",
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_140_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-14.0",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        context "when migrating a valid Scottish RdSAP-Schema-S-12.0 assessment" do
          expected_sap_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2008-11-17",
            "date_registered" => "2008-11-17",
            "type_of_assessment" => "RdSAP",
            "current_energy_efficiency_rating" => 32,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2018-11-16",
            "address_line1" => "1 Some Street",
            "address_line2" => "",
            "address_line3" => "",
            "address_line4" => "",
            "town" => "Newkirk",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000000",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_rdsap_120_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "RdSAP-Schema-S-12.0",
                                                 migrated: "true"

            rdsap_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(rdsap_data).to eq expected_sap_assessment_data
          end
        end

        it "rejects a migration with an incorrect schema name" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_xml,
            accepted_responses: [403],
            scopes: %w[migrate:scotland],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-19.0",
            migrated: true,
          ).status).to eq(403)
        end
      end

      context "when migrating a valid Scottish CEPC assessment" do
        context "when migrating a CEPC-S-7.1" do
          expected_cepc_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2023-06-27",
            "date_registered" => "2023-06-27",
            "type_of_assessment" => "CEPC",
            "current_energy_efficiency_rating" => 120,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2033-06-26",
            "address_line1" => "Non-dom Property",
            "address_line2" => "Some Street",
            "address_line3" => "Bigger Line",
            "address_line4" => nil,
            "town" => "Town",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000001",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_cepc_71_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "CEPC-S-7.1",
                                                 migrated: "true"

            cepc_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(cepc_data).to eq expected_cepc_assessment_data
          end
        end

        context "when migrating a CEPC-S-7.0" do
          expected_cepc_assessment_data = {
            "assessment_id" => "0000-0000-0000-0000-0000",
            "date_of_assessment" => "2015-02-02",
            "date_registered" => "2015-02-05",
            "type_of_assessment" => "CEPC",
            "current_energy_efficiency_rating" => 119,
            "postcode" => "FK1 1XE",
            "date_of_expiry" => "2025-02-04",
            "address_line1" => "Non-dom Property",
            "address_line2" => "Some Street",
            "address_line3" => "Bigger Line",
            "address_line4" => nil,
            "town" => "Town",
            "scheme_assessor_id" => "SPEC000000",
            "opt_out" => false,
            "address_id" => "LPRN-0000000001",
            "migrated" => true,
            "cancelled_at" => nil,
            "not_for_issue_at" => nil,
            "created_at" => "2023-06-27",
            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
          }

          it "successfully migrates the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_cepc_70_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[migrate:scotland],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "CEPC-S-7.0",
                                                 migrated: "true"

            cepc_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(cepc_data).to eq expected_cepc_assessment_data
          end
        end
      end

      context "when migrating a valid Scottish Action Plan assessment" do
        expected_action_plan_assessment_data = {
          "assessment_id" => "0000-0000-0000-0000-0000",
          "date_of_assessment" => "2023-06-27",
          "date_registered" => "2023-06-27",
          "type_of_assessment" => "CS63",
          "current_energy_efficiency_rating" => 0,
          "postcode" => "FK1 1XE",
          "date_of_expiry" => "2026-12-27",
          "address_line1" => "Non-dom Property",
          "address_line2" => "",
          "address_line3" => "",
          "address_line4" => "",
          "town" => "Town",
          "scheme_assessor_id" => "SPEC000000",
          "opt_out" => false,
          "address_id" => "0000000001",
          "migrated" => true,
          "cancelled_at" => nil,
          "not_for_issue_at" => nil,
          "created_at" => "2023-06-27",
          "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
        }

        it "successfully migrates the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_action_plan_xml,
                                               accepted_responses: [201],
                                               scopes: %w[migrate:scotland],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "CS63-S-7.0",
                                               migrated: "true"

          action_plan_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(action_plan_data).to eq expected_action_plan_assessment_data
        end
      end

      context "when migrating a valid Scottish DEC assessment" do
        expected_dec_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0000",
                                         "date_of_assessment" => "2023-06-27",
                                         "date_registered" => "2023-06-27",
                                         "type_of_assessment" => "DEC",
                                         "current_energy_efficiency_rating" => 0,
                                         "postcode" => "EH14 2SP",
                                         "date_of_expiry" => "2026-03-18",
                                         "address_line1" => "Non-dom Property",
                                         "address_line2" => "Buisness Park",
                                         "address_line3" => "",
                                         "address_line4" => "",
                                         "town" => "Town",
                                         "scheme_assessor_id" => "SPEC000000",
                                         "opt_out" => false,
                                         "address_id" => "0000000001",
                                         "migrated" => true,
                                         "cancelled_at" => nil,
                                         "not_for_issue_at" => nil,
                                         "created_at" => "2023-06-27",
                                         "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a" }

        it "successfully migrates the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_dec_70_xml,
                                               accepted_responses: [201],
                                               scopes: %w[migrate:scotland],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "DECAR-S-7.0",
                                               migrated: "true"

          dec_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(dec_data).to eq expected_dec_assessment_data
        end
      end

      context "when migrating a valid Scottish DEC-AR assessment" do
        expected_dec_ar_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0000",
                                            "date_of_assessment" => "2023-06-27",
                                            "date_registered" => "2023-06-27",
                                            "type_of_assessment" => "DEC-AR",
                                            "current_energy_efficiency_rating" => 0,
                                            "postcode" => "EH14 2SP",
                                            "date_of_expiry" => "2033-06-26",
                                            "address_line1" => "Non-dom Property",
                                            "address_line2" => "Buisness Park",
                                            "address_line3" => "",
                                            "address_line4" => "",
                                            "town" => "Town",
                                            "scheme_assessor_id" => "SPEC000000",
                                            "opt_out" => false,
                                            "address_id" => "0000000001",
                                            "migrated" => true,
                                            "cancelled_at" => nil,
                                            "not_for_issue_at" => nil,
                                            "created_at" => "2023-06-27",
                                            "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a" }

        it "successfully migrates the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_dec_ar_70_xml,
                                               accepted_responses: [201],
                                               scopes: %w[migrate:scotland],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "DECAR-S-7.0",
                                               migrated: "true"

          dec_ar_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(dec_ar_data).to eq expected_dec_ar_assessment_data
        end
      end

      context "when migrating a valid Scottish DEC+AR assessment" do
        expected_dec_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0040",
                                         "date_of_assessment" => "2023-06-27",
                                         "date_registered" => "2023-06-27",
                                         "type_of_assessment" => "DEC",
                                         "current_energy_efficiency_rating" => 0,
                                         "postcode" => "EH14 2SP",
                                         "date_of_expiry" => "2035-03-31",
                                         "address_line1" => "Non-dom Property",
                                         "address_line2" => "Buisness Park",
                                         "address_line3" => "",
                                         "address_line4" => "",
                                         "town" => "Town",
                                         "scheme_assessor_id" => "SPEC000000",
                                         "opt_out" => false,
                                         "address_id" => "0000000001",
                                         "migrated" => true,
                                         "cancelled_at" => nil,
                                         "not_for_issue_at" => nil,
                                         "created_at" => "2023-06-27",
                                         "hashed_assessment_id" => "e2a73bb7e6874fb5f0b4541326bbbc8cca6d375012da903b2a9712e2edaa12ee" }
        expected_dec_ar_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0050",
                                            "date_of_assessment" => "2023-06-27",
                                            "date_registered" => "2023-06-27",
                                            "type_of_assessment" => "DEC-AR",
                                            "current_energy_efficiency_rating" => 0,
                                            "postcode" => "EH14 2SP",
                                            "date_of_expiry" => "2033-06-26",
                                            "address_line1" => "Non-dom Property",
                                            "address_line2" => "Buisness Park",
                                            "address_line3" => "",
                                            "address_line4" => "",
                                            "town" => "Town",
                                            "scheme_assessor_id" => "SPEC000000",
                                            "opt_out" => false,
                                            "address_id" => "0000000001",
                                            "migrated" => true,
                                            "cancelled_at" => nil,
                                            "not_for_issue_at" => nil,
                                            "created_at" => "2023-06-27",
                                            "hashed_assessment_id" => "e81c2c444c8afe8cf6554bd26ece29405a43ea5ac7d1b02e1737bba5a5bd14c0" }
        expected_linked_assessment_data = [{ "assessment_id" => "0000-0000-0000-0000-0040", "linked_assessment_id" => "0000-0000-0000-0000-0050" }]

        it "successfully migrates the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_dec_and_ar_70_xml,
                                               accepted_responses: [201],
                                               scopes: %w[migrate:scotland],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "DECAR-S-7.0",
                                               migrated: "true"

          dec_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0040'",
          ).entries.first

          dec_ar_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0050'",
          ).entries.first

          linked_assessments = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.linked_assessments WHERE assessment_id = '0000-0000-0000-0000-0040'",
          ).entries

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0040"
          expect(dec_ar_data).to eq expected_dec_ar_assessment_data
          expect(dec_data).to eq expected_dec_assessment_data
          expect(linked_assessments).to eq expected_linked_assessment_data
        end
      end

      context "when migrating a non-Scottish domestic assessment" do
        it "rejects a migration" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_not_scottish_xml,
            accepted_responses: [403],
            scopes: %w[migrate:scotland],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-19.0",
            migrated: true,
          ).status).to eq(403)
        end

        it "rejects a migration when the schema name incorrectly indicates it is Scottish" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_not_scottish_xml,
            accepted_responses: [400],
            scopes: %w[migrate:scotland],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-S-19.0",
            migrated: true,
          ).status).to eq(400)
        end
      end

      context "when attempting to lodge a Scottish domestic assessment" do
        it "rejects the Scottish assessment" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_xml,
            accepted_responses: [403],
            scopes: %w[migrate:scotland],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-S-19.0",
            migrated: false,
          ).status).to eq(403)
        end

        it "rejects the non-Scottish assessment" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_not_scottish_xml,
            accepted_responses: [403],
            scopes: %w[migrate:scotland],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-19.0",
            migrated: false,
          ).status).to eq(403)
        end
      end
    end

    context "when the client does not have a migrate:scotland role" do
      context "when the client has a migrate assessment role" do
        it "rejects a migration of a Scottish domestic assessment" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_xml,
            accepted_responses: [403],
            scopes: %w[migrate:assessment],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-S-19.0",
            migrated: true,
          ).status).to eq(403)
        end
      end

      context "when the client has a lodgement role" do
        it "rejects a migration from a client with a lodgement role" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_xml,
            accepted_responses: [403],
            scopes: %w[assessment:lodge],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-S-19.0",
            migrated: true,
          ).status).to eq(403)
        end
      end
    end

    context "when the client has a lodge Scotland role"  do
      context "when attempting to migrate an assessment" do
        it "rejects the Scottish assessment" do
          response = JSON.parse(lodge_scottish_assessment(
            assessment_body: valid_rdsap_xml,
            accepted_responses: [403],
            scopes: %w[scotland_assessment:lodge],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-S-19.0",
            migrated: true,
          ).body,
                                symbolize_names: true)

          expect(response[:errors][0][:title]).to eq "You are not authorised to perform this migration request"
        end
      end

      context "when lodging a Scotland domestic assessment" do
        let(:lodged_scotland_rdsap_qualification_data) do
          ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first
        end

        it "rejects the non-Scottish assessment" do
          response = JSON.parse(lodge_scottish_assessment(
            assessment_body: valid_rdsap_not_scottish_xml,
            accepted_responses: [403],
            scopes: %w[scotland_assessment:lodge],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-19.0",
            migrated: false,
          ).body,
                                symbolize_names: true)

          expect(response[:errors][0][:title]).to eq "Only Scottish schemas are supported"
        end

        context "when the RdSAP xml is valid with the correct Scottish schema" do
          it "successfully lodges the assessment" do
            expect(lodge_scottish_assessment(
              assessment_body: valid_rdsap_xml,
              accepted_responses: [201],
              scopes: %w[scotland_assessment:lodge],
              auth_data: {
                scheme_ids: [scheme_id],
              },
              schema_name: "RdSAP-Schema-S-19.0",
              migrated: false,
            ).status).to eq(201)
          end

          it "has all expected data points present" do
            expected_rdsap_data = {
              "assessment_id" => "0000-0000-0000-0000-0000",
              "date_of_assessment" => "2023-06-27",
              "date_registered" => "2023-06-27",
              "type_of_assessment" => "RdSAP",
              "current_energy_efficiency_rating" => 79,
              "postcode" => "FK1 1XE",
              "date_of_expiry" => "2033-06-26",
              "address_line1" => "1 Some Street",
              "address_line2" => "",
              "address_line3" => "",
              "address_line4" => "",
              "town" => "Newkirk",
              "scheme_assessor_id" => "SPEC000000",
              "opt_out" => false,
              "address_id" => "LPRN-0000000000",
              "migrated" => false,
              "cancelled_at" => nil,
              "not_for_issue_at" => nil,
              "created_at" => "2023-06-27",
              "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
            }

            response = lodge_scottish_assessment               assessment_body: valid_rdsap_xml,
                                                               accepted_responses: [201],
                                                               scopes: %w[scotland_assessment:lodge],
                                                               auth_data: {
                                                                 scheme_ids: [scheme_id],
                                                               },
                                                               schema_name: "RdSAP-Schema-S-19.0",
                                                               migrated: false

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(lodged_scotland_rdsap_qualification_data).to eq expected_rdsap_data
          end

          context "when lodging the same assessment ID" do
            before do
              lodge_scottish_assessment assessment_body: valid_rdsap_xml,
                                        accepted_responses: [201],
                                        scopes: %w[scotland_assessment:lodge],
                                        auth_data: {
                                          scheme_ids: [scheme_id],
                                        },
                                        schema_name: "RdSAP-Schema-S-19.0",
                                        migrated: false
            end

            it "raises an error" do
              expect(lodge_scottish_assessment(
                assessment_body: valid_rdsap_xml,
                accepted_responses: [409],
                scopes: %w[scotland_assessment:lodge],
                auth_data: {
                  scheme_ids: [scheme_id],
                },
                schema_name: "RdSAP-Schema-S-19.0",
                migrated: false,
              ).status).to eq(409)
            end
          end

          context "when lodging an assessment submitted by an assessor who is unqualified" do
            let(:rdsap_xml) do
              add_assessor scheme_id:,
                           assessor_id: "UNQU000000",
                           body: AssessorStub.new.fetch_request_body(
                             scotland_rdsap: "INACTIVE",
                           )

              xml = Nokogiri.XML valid_rdsap_xml

              xml.css("Membership-Number").children.first.content = "UNQU000000"

              xml.to_s
            end

            it "does not lodge the assessment" do
              expect(lodge_scottish_assessment(
                assessment_body: rdsap_xml,
                accepted_responses: [400],
                scopes: %w[scotland_assessment:lodge],
                auth_data: {
                  scheme_ids: [scheme_id],
                },
                schema_name: "RdSAP-Schema-S-19.0",
                migrated: false,
              ).status).to eq(400)
            end
          end
        end

        context "when the SAP xml is valid with the correct Scottish schema" do
          let(:lodged_scotland_sap_qualification_data) do
            ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first
          end

          it "has all expected data points present" do
            expected_sap_data = {
              "assessment_id" => "0000-0000-0000-0000-0000",
              "date_of_assessment" => "2023-06-27",
              "date_registered" => "2023-06-27",
              "type_of_assessment" => "SAP",
              "current_energy_efficiency_rating" => 91,
              "postcode" => "EH1 2NG",
              "date_of_expiry" => "2033-06-26",
              "address_line1" => "1 LOVELY ROAD",
              "address_line2" => "NICE ESTATE",
              "address_line3" => "",
              "address_line4" => nil,
              "town" => "TOWN",
              "scheme_assessor_id" => "SPEC000000",
              "opt_out" => false,
              "address_id" => "0000000001",
              "migrated" => false,
              "cancelled_at" => nil,
              "not_for_issue_at" => nil,
              "created_at" => "2023-06-27",
              "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
            }

            response = lodge_scottish_assessment               assessment_body: valid_sap_xml,
                                                               accepted_responses: [201],
                                                               scopes: %w[scotland_assessment:lodge],
                                                               auth_data: {
                                                                 scheme_ids: [scheme_id],
                                                               },
                                                               schema_name: "SAP-Schema-S-19.0.0",
                                                               migrated: false

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(lodged_scotland_sap_qualification_data).to eq expected_sap_data
          end
        end
      end

      context "when lodging a valid Scottish RdSAP-Schema-S-22.0.0 assessment" do
        expected_rdsap_assessment_data = {
          "assessment_id" => "0000-0000-0000-0000-0000",
          "date_of_assessment" => "2023-06-27",
          "date_registered" => "2023-06-27",
          "type_of_assessment" => "RdSAP",
          "current_energy_efficiency_rating" => 80,
          "postcode" => "FK1 1XE",
          "date_of_expiry" => "2033-06-26",
          "address_line1" => "1 Some Street",
          "address_line2" => "",
          "address_line3" => "",
          "address_line4" => "",
          "town" => "Newkirk",
          "scheme_assessor_id" => "SPEC000000",
          "opt_out" => false,
          "address_id" => "UPRN-0000000000",
          "migrated" => false,
          "cancelled_at" => nil,
          "not_for_issue_at" => nil,
          "created_at" => "2023-06-27",
          "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
        }

        it "successfully lodges the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_rdsap_2200_xml,
                                               accepted_responses: [201],
                                               scopes: %w[scotland_assessment:lodge],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "RdSAP-Schema-S-22.0.0",
                                               migrated: false

          rdsap_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(rdsap_data).to eq expected_rdsap_assessment_data
        end
      end

      context "when lodging a valid Scottish SAP-Schema-S-20.0.0 assessment" do
        expected_sap_assessment_data = {
          "assessment_id" => "0000-0000-0000-0000-0000",
          "date_of_assessment" => "2023-06-27",
          "date_registered" => "2023-06-27",
          "type_of_assessment" => "SAP",
          "current_energy_efficiency_rating" => 91,
          "postcode" => "EH1 2NG",
          "date_of_expiry" => "2033-06-26",
          "address_line1" => "1 LOVELY ROAD",
          "address_line2" => "NICE ESTATE",
          "address_line3" => "",
          "address_line4" => nil,
          "town" => "TOWN",
          "scheme_assessor_id" => "SPEC000000",
          "opt_out" => false,
          "address_id" => "UPRN-0000000001",
          "migrated" => false,
          "cancelled_at" => nil,
          "not_for_issue_at" => nil,
          "created_at" => "2023-06-27",
          "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
        }

        it "successfully lodges the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_sap_2000_xml,
                                               accepted_responses: [201],
                                               scopes: %w[scotland_assessment:lodge],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "SAP-Schema-S-20.0.0",
                                               migrated: false

          rdsap_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(rdsap_data).to eq expected_sap_assessment_data
        end
      end

      context "when lodging a valid Scottish CEPC-S-8.0.0 assessment" do
        expected_cepc_assessment_data = {
          "assessment_id" => "0000-0000-0000-0000-0000",
          "date_of_assessment" => "2023-07-11",
          "date_registered" => "2023-08-04",
          "type_of_assessment" => "CEPC",
          "current_energy_efficiency_rating" => 120,
          "postcode" => "FK1 1XE",
          "date_of_expiry" => "2033-08-03",
          "address_line1" => "Non-dom Property",
          "address_line2" => "Some Street",
          "address_line3" => "Bigger Line",
          "address_line4" => nil,
          "town" => "Town",
          "scheme_assessor_id" => "SPEC000000",
          "opt_out" => false,
          "address_id" => "UPRN-0000000001",
          "migrated" => false,
          "cancelled_at" => nil,
          "not_for_issue_at" => nil,
          "created_at" => "2023-06-27",
          "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
        }

        it "successfully lodges the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_cepc_800_xml,
                                               accepted_responses: [201],
                                               scopes: %w[scotland_assessment:lodge],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "CEPC-S-8.0.0",
                                               migrated: false

          cepc_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(cepc_data).to eq expected_cepc_assessment_data
        end
      end

      context "when lodging a valid Scottish CEPC-S-7.1 assessment" do
        expected_cepc_assessment_data = {
          "assessment_id" => "0000-0000-0000-0000-0000",
          "date_of_assessment" => "2023-06-27",
          "date_registered" => "2023-06-27",
          "type_of_assessment" => "CEPC",
          "current_energy_efficiency_rating" => 120,
          "postcode" => "FK1 1XE",
          "date_of_expiry" => "2033-06-26",
          "address_line1" => "Non-dom Property",
          "address_line2" => "Some Street",
          "address_line3" => "Bigger Line",
          "address_line4" => nil,
          "town" => "Town",
          "scheme_assessor_id" => "SPEC000000",
          "opt_out" => false,
          "address_id" => "LPRN-0000000001",
          "migrated" => false,
          "cancelled_at" => nil,
          "not_for_issue_at" => nil,
          "created_at" => "2023-06-27",
          "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
        }

        it "successfully lodges the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_cepc_71_xml,
                                               accepted_responses: [201],
                                               scopes: %w[scotland_assessment:lodge],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "CEPC-S-7.1",
                                               migrated: false

          cepc_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(cepc_data).to eq expected_cepc_assessment_data
        end
      end

      context "when lodging a valid Scottish Action Plan 8.0.0 assessment" do
        expected_action_plan_assessment_data = {
          "assessment_id" => "0000-0000-0000-0000-0000",
          "date_of_assessment" => "2025-06-04",
          "date_registered" => "2025-06-11",
          "type_of_assessment" => "CS63",
          "current_energy_efficiency_rating" => 0,
          "postcode" => "FK1 1XE",
          "date_of_expiry" => "2028-12-11",
          "address_line1" => "Non-dom Property",
          "address_line2" => "",
          "address_line3" => "",
          "address_line4" => "",
          "town" => "Town",
          "scheme_assessor_id" => "SPEC000000",
          "opt_out" => false,
          "address_id" => "UPRN-0000000001",
          "migrated" => false,
          "cancelled_at" => nil,
          "not_for_issue_at" => nil,
          "created_at" => "2023-06-27",
          "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a",
        }

        it "successfully lodges the assessment" do
          response = lodge_scottish_assessment assessment_body: valid_action_plan_800_xml,
                                               accepted_responses: [201],
                                               scopes: %w[scotland_assessment:lodge],
                                               auth_data: {
                                                 scheme_ids: [scheme_id],
                                               },
                                               schema_name: "CS63-S-8.0.0",
                                               migrated: false

          action_plan_data = ActiveRecord::Base.connection.exec_query(
            "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
          ).entries.first

          expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
          expect(action_plan_data).to eq expected_action_plan_assessment_data
        end
      end

      describe "when lodging valid DECAR-S-8.0.0 documents" do
        context "when lodging a valid Scottish DEC assessment" do
          expected_dec_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0000",
                                           "date_of_assessment" => "2025-04-10",
                                           "date_registered" => "2025-04-10",
                                           "type_of_assessment" => "DEC",
                                           "current_energy_efficiency_rating" => 0,
                                           "postcode" => "EH14 2SP",
                                           "date_of_expiry" => "2026-03-18",
                                           "address_line1" => "Non-dom Property",
                                           "address_line2" => "Buisness Park",
                                           "address_line3" => "",
                                           "address_line4" => "",
                                           "town" => "Town",
                                           "scheme_assessor_id" => "SPEC000000",
                                           "opt_out" => false,
                                           "address_id" => "UPRN-0000000001",
                                           "migrated" => false,
                                           "cancelled_at" => nil,
                                           "not_for_issue_at" => nil,
                                           "created_at" => "2023-06-27",
                                           "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a" }

          it "successfully lodges the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_dec_800_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[scotland_assessment:lodge],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "DECAR-S-8.0.0",
                                                 migrated: false

            dec_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(dec_data).to eq expected_dec_assessment_data
          end
        end

        context "when lodging a valid Scottish DEC-AR assessment" do
          expected_dec_ar_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0000",
                                              "date_of_assessment" => "2019-10-21",
                                              "date_registered" => "2019-11-22",
                                              "type_of_assessment" => "DEC-AR",
                                              "current_energy_efficiency_rating" => 0,
                                              "postcode" => "EH14 2SP",
                                              "date_of_expiry" => "2029-11-21",
                                              "address_line1" => "Non-dom Property",
                                              "address_line2" => "Buisness Park",
                                              "address_line3" => "",
                                              "address_line4" => "",
                                              "town" => "Town",
                                              "scheme_assessor_id" => "SPEC000000",
                                              "opt_out" => false,
                                              "address_id" => "UPRN-0000000001",
                                              "migrated" => false,
                                              "cancelled_at" => nil,
                                              "not_for_issue_at" => nil,
                                              "created_at" => "2023-06-27",
                                              "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a" }

          it "successfully lodges the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_dec_ar_800_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[scotland_assessment:lodge],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "DECAR-S-8.0.0",
                                                 migrated: false

            dec_ar_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(dec_ar_data).to eq expected_dec_ar_assessment_data
          end
        end

        context "when lodging a valid Scottish DEC+AR assessment" do
          expected_dec_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0040",
                                           "date_of_assessment" => "2023-06-27",
                                           "date_registered" => "2023-06-27",
                                           "type_of_assessment" => "DEC",
                                           "current_energy_efficiency_rating" => 0,
                                           "postcode" => "EH14 2SP",
                                           "date_of_expiry" => "2035-03-31",
                                           "address_line1" => "Non-dom Property",
                                           "address_line2" => "Buisness Park",
                                           "address_line3" => "",
                                           "address_line4" => "",
                                           "town" => "Town",
                                           "scheme_assessor_id" => "SPEC000000",
                                           "opt_out" => false,
                                           "address_id" => "UPRN-0000000001",
                                           "migrated" => false,
                                           "cancelled_at" => nil,
                                           "not_for_issue_at" => nil,
                                           "created_at" => "2023-06-27",
                                           "hashed_assessment_id" => "e2a73bb7e6874fb5f0b4541326bbbc8cca6d375012da903b2a9712e2edaa12ee" }
          expected_dec_ar_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0050",
                                              "date_of_assessment" => "2023-06-27",
                                              "date_registered" => "2023-06-27",
                                              "type_of_assessment" => "DEC-AR",
                                              "current_energy_efficiency_rating" => 0,
                                              "postcode" => "EH14 2SP",
                                              "date_of_expiry" => "2033-06-26",
                                              "address_line1" => "Non-dom Property",
                                              "address_line2" => "Buisness Park",
                                              "address_line3" => "",
                                              "address_line4" => "",
                                              "town" => "Town",
                                              "scheme_assessor_id" => "SPEC000000",
                                              "opt_out" => false,
                                              "address_id" => "UPRN-0000000001",
                                              "migrated" => false,
                                              "cancelled_at" => nil,
                                              "not_for_issue_at" => nil,
                                              "created_at" => "2023-06-27",
                                              "hashed_assessment_id" => "e81c2c444c8afe8cf6554bd26ece29405a43ea5ac7d1b02e1737bba5a5bd14c0" }
          expected_linked_assessment_data = [{ "assessment_id" => "0000-0000-0000-0000-0040", "linked_assessment_id" => "0000-0000-0000-0000-0050" }]

          it "successfully lodges the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_dec_and_ar_800_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[scotland_assessment:lodge],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "DECAR-S-8.0.0",
                                                 migrated: false

            dec_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0040'",
            ).entries.first

            dec_ar_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0050'",
            ).entries.first

            linked_assessments = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.linked_assessments WHERE assessment_id = '0000-0000-0000-0000-0040'",
            ).entries

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0040"
            expect(dec_ar_data).to eq expected_dec_ar_assessment_data
            expect(dec_data).to eq expected_dec_assessment_data
            expect(linked_assessments).to eq expected_linked_assessment_data
          end
        end
      end

      describe "when lodging valid DECAR-S-7.0 documents" do
        context "when lodging a valid Scottish DEC assessment" do
          expected_dec_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0000",
                                           "date_of_assessment" => "2023-06-27",
                                           "date_registered" => "2023-06-27",
                                           "type_of_assessment" => "DEC",
                                           "current_energy_efficiency_rating" => 0,
                                           "postcode" => "EH14 2SP",
                                           "date_of_expiry" => "2026-03-18",
                                           "address_line1" => "Non-dom Property",
                                           "address_line2" => "Buisness Park",
                                           "address_line3" => "",
                                           "address_line4" => "",
                                           "town" => "Town",
                                           "scheme_assessor_id" => "SPEC000000",
                                           "opt_out" => false,
                                           "address_id" => "0000000001",
                                           "migrated" => false,
                                           "cancelled_at" => nil,
                                           "not_for_issue_at" => nil,
                                           "created_at" => "2023-06-27",
                                           "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a" }

          it "successfully lodges the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_dec_70_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[scotland_assessment:lodge],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "DECAR-S-7.0",
                                                 migrated: false

            dec_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(dec_data).to eq expected_dec_assessment_data
          end
        end

        context "when lodging a valid Scottish DEC-AR assessment" do
          expected_dec_ar_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0000",
                                              "date_of_assessment" => "2023-06-27",
                                              "date_registered" => "2023-06-27",
                                              "type_of_assessment" => "DEC-AR",
                                              "current_energy_efficiency_rating" => 0,
                                              "postcode" => "EH14 2SP",
                                              "date_of_expiry" => "2033-06-26",
                                              "address_line1" => "Non-dom Property",
                                              "address_line2" => "Buisness Park",
                                              "address_line3" => "",
                                              "address_line4" => "",
                                              "town" => "Town",
                                              "scheme_assessor_id" => "SPEC000000",
                                              "opt_out" => false,
                                              "address_id" => "0000000001",
                                              "migrated" => false,
                                              "cancelled_at" => nil,
                                              "not_for_issue_at" => nil,
                                              "created_at" => "2023-06-27",
                                              "hashed_assessment_id" => "4af9d2c31cf53e72ef6f59d3f59a1bfc500ebc2b1027bc5ca47361435d988e1a" }

          it "successfully lodges the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_dec_ar_70_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[scotland_assessment:lodge],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "DECAR-S-7.0",
                                                 migrated: false

            dec_ar_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0000'",
            ).entries.first

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0000"
            expect(dec_ar_data).to eq expected_dec_ar_assessment_data
          end
        end

        context "when lodging a valid Scottish DEC+AR assessment" do
          expected_dec_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0040",
                                           "date_of_assessment" => "2023-06-27",
                                           "date_registered" => "2023-06-27",
                                           "type_of_assessment" => "DEC",
                                           "current_energy_efficiency_rating" => 0,
                                           "postcode" => "EH14 2SP",
                                           "date_of_expiry" => "2035-03-31",
                                           "address_line1" => "Non-dom Property",
                                           "address_line2" => "Buisness Park",
                                           "address_line3" => "",
                                           "address_line4" => "",
                                           "town" => "Town",
                                           "scheme_assessor_id" => "SPEC000000",
                                           "opt_out" => false,
                                           "address_id" => "0000000001",
                                           "migrated" => false,
                                           "cancelled_at" => nil,
                                           "not_for_issue_at" => nil,
                                           "created_at" => "2023-06-27",
                                           "hashed_assessment_id" => "e2a73bb7e6874fb5f0b4541326bbbc8cca6d375012da903b2a9712e2edaa12ee" }
          expected_dec_ar_assessment_data = { "assessment_id" => "0000-0000-0000-0000-0050",
                                              "date_of_assessment" => "2023-06-27",
                                              "date_registered" => "2023-06-27",
                                              "type_of_assessment" => "DEC-AR",
                                              "current_energy_efficiency_rating" => 0,
                                              "postcode" => "EH14 2SP",
                                              "date_of_expiry" => "2033-06-26",
                                              "address_line1" => "Non-dom Property",
                                              "address_line2" => "Buisness Park",
                                              "address_line3" => "",
                                              "address_line4" => "",
                                              "town" => "Town",
                                              "scheme_assessor_id" => "SPEC000000",
                                              "opt_out" => false,
                                              "address_id" => "0000000001",
                                              "migrated" => false,
                                              "cancelled_at" => nil,
                                              "not_for_issue_at" => nil,
                                              "created_at" => "2023-06-27",
                                              "hashed_assessment_id" => "e81c2c444c8afe8cf6554bd26ece29405a43ea5ac7d1b02e1737bba5a5bd14c0" }
          expected_linked_assessment_data = [{ "assessment_id" => "0000-0000-0000-0000-0040", "linked_assessment_id" => "0000-0000-0000-0000-0050" }]

          it "successfully lodges the assessment" do
            response = lodge_scottish_assessment assessment_body: valid_dec_and_ar_70_xml,
                                                 accepted_responses: [201],
                                                 scopes: %w[scotland_assessment:lodge],
                                                 auth_data: {
                                                   scheme_ids: [scheme_id],
                                                 },
                                                 schema_name: "DECAR-S-7.0",
                                                 migrated: false

            dec_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0040'",
            ).entries.first

            dec_ar_data = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.assessments WHERE assessment_id = '0000-0000-0000-0000-0050'",
            ).entries.first

            linked_assessments = ActiveRecord::Base.connection.exec_query(
              "SELECT * FROM scotland.linked_assessments WHERE assessment_id = '0000-0000-0000-0000-0040'",
            ).entries

            expect(JSON.parse(response.body, symbolize_names: true)[:data][:assessments].first).to eq "0000-0000-0000-0000-0040"
            expect(dec_ar_data).to eq expected_dec_ar_assessment_data
            expect(dec_data).to eq expected_dec_assessment_data
            expect(linked_assessments).to eq expected_linked_assessment_data
          end
        end
      end

      context "when lodging a non-Scottish domestic assessment" do
        it "rejects a lodgement" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_not_scottish_xml,
            accepted_responses: [403],
            scopes: %w[scotland_assessment:lodge],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-19.0",
            migrated: false,
          ).status).to eq(403)
        end

        it "rejects a lodgement when the schema name incorrectly indicates it is Scottish" do
          expect(lodge_scottish_assessment(
            assessment_body: valid_rdsap_not_scottish_xml,
            accepted_responses: [400],
            scopes: %w[scotland_assessment:lodge],
            auth_data: {
              scheme_ids: [scheme_id],
            },
            schema_name: "RdSAP-Schema-S-19.0",
            migrated: false,
          ).status).to eq(400)
        end
      end
    end
  end
end
