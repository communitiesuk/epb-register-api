describe "Acceptance::LodgementRules" do
  ...
  let(:scheme_id) do
    ...
    add_assessor(
      scheme_id,
      "SPEC000000",
      fetch_assessor_stub.fetch_request_body(nonDomesticNos3: "ACTIVE", domesticRdSap: "ACTIVE"),
    )
    ...
    context "that breaks a rule" do
      it "should reject the assessment" do
        ...
        expect(JSON.parse(response.body,
                          symbolize_names: true)[:errors])
            .to eq([{code: "MUST_HAVE_HABITABLE_ROOMS",
                     title: "\"Habitable-Room-Count\" must be an integer and must be greater than or equal to 1"}])
      end
    end
