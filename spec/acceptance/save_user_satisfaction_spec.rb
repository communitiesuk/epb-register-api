describe "Acceptance:SaveUserSatisfaction" do
  ...
  describe "put /api/user-satisfaction/ " do
    context "when data send in the put is well formed" do
      ...
      it "returns a 200 correct status" do
        put_user_satisfaction(body: data, scopes: %w[admin:upload_stats])
      end

      it "the send data is saved to the database" do
        put_user_satisfaction(body: data, scopes: %w[admin:upload_stats])
        saved_data = ActiveRecord::Base.connection.exec_query("SELECT * FROM user_satisfaction WHERE month ='2021-09-01'").first
    ...
    context "when send data has an invalid date" do
      it "returns a 401  status" do
        data = { "stats_date" => "hello",
                 ...
                 "dissatisfied" => 4,
                 "very_dissatisfied" => 5 }
        response = put_user_satisfaction(body: data, scopes: %w[admin:upload_stats], accepted_responses: [400])
    ...
    context "when request does not have the correct scopes" do
      it "returns a 403 status for forbidden" do
        put_user_satisfaction(body: "data", scopes: %w[assessment:search], accepted_responses: [403])
      end
    end
  end
end
